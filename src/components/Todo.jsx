        ...
        <div className={`flex items-baseline justify-between ${isChecked ? 'bg-green-700' : 'bg-gray-800'} text-white py-5 px-4 rounded-md mb-4`}>
            <div className='flex items-baseline gap-3'>
                <input
                    ...
                    className="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded cursor-pointer focus:ring-blue-500 focus:ring-2"
                />
                <div className='flex flex-col items-start'>
                    <p className='text-xl text-white'>{task.task}</p>
                    <p className={`text-lg ${isChecked ? "text-black" : "text-gray-400"}`}>{task.des}</p>
                </div>
            </div>
            <div className='flex items-center gap-x-4'>
                {isChecked ? <span className='text-green-300'>Completed</span> :
